...
> 克隆的仓库默认设置了将地`master`分支跟踪远程的`master`分支。
...
> `git push`默认将当前`master`分支推送到`origin`的`master`分支。只有当拥有远程仓库的写入权限，并且之前没有人推送过时，这条命令才能生效。
...
> 设置命令别名时，包含空白符的命令需要加引号，单引号`''`包含双引号`""`；非git命令需要在命令前加`!`，并还有完整的git命令。

## 分支

在新建Git仓库时会默认创建一个`master`分支。`HEAD`指针指向当前所在的本地分支，可看做当前分支的别名。

暂存时，Git会为每一个文件计算校验和，把当前版本的文件快照以blob对象保存到Git仓库中，把校验和加入到暂存区。提交时，Git会计算每个子目录的校验和，将Git仓库中的校验和保存为树结构，之后Git创建一个恶提交对象，包含了作者名、邮箱、提交信息和树对象。新的提交对象有一个指向上次提交对象的指针。
